...
//                res.sendFile(path.resolve(__dirname+"/../views/chat1.html"));
    ...
    io.on('connection',function(socket){
        ...
        models.user.find({"username" : username},{friends:1,_id:0},function(err,doc){
            if(err){res.json(err);}
            else{
                friends=[];
                pending=[];
    ...
    app.post('/friend_request',function(req,res){
        ...
        res.setHeader("Access-Control-Allow-Method","'GET, POST, OPTIONS, PUT, PATCH, DELETE'");
        friend=true;
        models.user.find({"username" : req.body.my_username,"friends.name":req.body.friend_username},function(err,doc){
            if(err){res.json(err);}
                ...
                console.log("Friend request : "+doc.length);
                models.user.update({
                    username:req.body.my_username
                },{
                    $push:{
                        friends:{
                            name: req.body.friend_username,
                            status: "Pending"
                        }
                    }
                },{
                ...
                io.to(users[req.body.friend_username]).emit('message', req.body);
            }
        });
    });
    ...
    app.post('/friend_request/confirmed',function(req,res){
        ...
        if(req.body.confirm=="Yes"){
            models.user.find({
                ...
                "friends.name":req.body.my_username
            },function(err,doc){
                if(err){
                    res.json(err);
                }
                    ...
                    models.user.update({
                        "username":req.body.my_username,
                        "friends.name":req.body.friend_username
                    },{
                        '$set':{
                        ...
                        if(err){res.json(err);}
                            ...
                            io.to(users[req.body.friend_username]).emit('friend', req.body.my_username);
                            io.to(users[req.body.my_username]).emit('friend', req.body.friend_username);
                        }
                    });
                    models.user.update({
                        username:req.body.friend_username
